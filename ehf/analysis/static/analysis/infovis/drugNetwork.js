    ...
    fd = new $jit.ForceDirected({
        ...
        Events: {
            ...
            onClick: function (node, eventInfo, e) {
                if (!node) return;
                if (node.nodeFrom) {
                    var html = '<b>Target Gene</b>:<br/>' + node.data.gene;
        ...
        onCreateLabel: function (domElement, node) {
            ...
            nameContainer.onmouseover = function () {
                ...
                //for species and drug node,it is different.
                if (node.data.nodeType == 'drug') {
                    var list = [];
                    node.eachAdjacency(function (adj) {
                        if (adj.getData('alpha')) list.push(adj.nodeTo.name + ' [target:' + adj.data.gene + ']');
                    });
                    if (list.length > 0)
                    ...
                    var approvedDrugList = [];
                    var otherDrugList = [];
                    node.eachAdjacency(function (adj) {
                        if (adj.nodeTo.getData('alpha') == 1) {
                            //console.log(adj.nodeTo.data.drugGroup);
                            if (adj.nodeTo.data.drugGroup == 'approved') {
                                approvedDrugList.push(adj.nodeTo.name + ' (' + adj.nodeTo.data.des + ')')
                            } else {
                                otherDrugList.push(adj.nodeTo.name + ' (' + adj.nodeTo.data.des + ')')
                            }
                        }
                    });
                    html = html + "<b> connections (" + (approvedDrugList.length + otherDrugList.length) + "):</b>";
                    if (approvedDrugList.length > 0) {
                        html = html + "<div class='approvedDrug'>Approved Drug (" + approvedDrugList.length + ")</div><ul><li>" + approvedDrugList.join("</li><li>") + "</li></ul>";
                    } else {
                        html = html + "<div class='approvedDrug'>Approved Drug (" + approvedDrugList.length + ")</div>&nbsp;No approved drugs";
                    }

                    if (otherDrugList.length > 0) {
                        html = html + "<div class='otherDrug'>Other Drug (" + otherDrugList.length + ")</div><ul><li>" + otherDrugList.join("</li><li>") + "</li></ul>";
                    } else {
                        html = html + "<div class='otherDrug'>Other Drug (" + otherDrugList.length + ")</div>&nbsp;No other drugs";
                    }
                    html = html + "<hr/>";
                }
                $jit.id('inner-detail').innerHTML = html;
            ...
            nameContainer.onclick = function () {
                if (node.data.nodeType == "drug") {
                    window.open('http://www.drugbank.ca/drugs/' + node.data.des, 'about:blank');
                }
            }
...
//add legend to list
    ...
    legend_div.innerHTML = '<div class="nodeDes">Node:&nbsp;&nbsp;<img src="/static/analysis/images/triangle1.png" width="12" height="12" /> pathogen &nbsp; &nbsp;' +
        '<img src="/static/analysis/images/circle.png" width="12" height="12" /> approved drug&nbsp; &nbsp;' +
        '<img src="/static/analysis/images/circle1.png" width="12" height="12" /> other drug&nbsp; &nbsp;</div>';

//zoom out
    fd.canvas.scale(1.2, 1.2);
...
function searchNet(data) {
    ...
    if (query.length) {
        ...
        fd.graph.eachNode(function (n) {
            ...
            if (name.toLowerCase() == query.toLowerCase() || des.toLowerCase() == query.toLowerCase()) {  //match exactly!
                found = 1;
                if (!node.selected) {
                    node.selected = true;
                    if (node.data.$type == 'star')
